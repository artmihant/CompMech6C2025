...
def show(ZSpaceTrajectory):
    ...
    def to_pixel(z):
        # Используем аргумент комплексного числа для раскраски бассейнов Ньютона
        return np.angle(z)
        # return abs(z)
    ...
    # Устанавливаем extent для правильного масштабирования осей
    extent = [X_min.real, X_max.real, Y_min.imag, Y_max.imag]
    im = ax.imshow(Image, cmap='hsv', extent=extent, origin='lower')

    # Настраиваем оси для показа комплексных координат
    ax.set_xlabel('Re(z)')
    ax.set_ylabel('Im(z)')
